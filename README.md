...
- **Resource groups** - point a group at "Health and power" instead, and it
  ...
  or both.
- **Unit frames** - a second way to show your health, power, and class resources, laid out the conventional way: a portrait (picture or live 3D model), a name and level header, a full-width bar each for health and power, and runes or combo points as a compact strip of segments rather than a row each. Numbers sit beside the bars or on them. Click a portrait or name to target that unit, right-click for its menu, and turning a frame on hides Blizzard's equivalent. Pick exactly which resources appear, set the bar look, height and opacity, and choose fonts and sizes for the name and the numbers. Independent of resource groups above, so use whichever reads better. Frames for you, your target, your target's target, your pet, your focus, and all four party slots, on the Frames tab
