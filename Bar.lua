...
function ns:CreateBarFrame(parent)
    ...
    -- the icon is hidden, and applies the resolved Stack Text Size /
    -- Stack Text Colour whenever they or the count change (ns:GetStackFontSize
    -- / ns:GetStackColor, Conditions.lua: bar override, then group, then the
    -- Visuals tab default). The template here only seeds the initial
    -- size/colour (matching the addon-wide default) so an inactive bar built
    -- before its first render still looks right; the font family itself is
    -- not the configured bar font, so the number stays legible whatever font
    -- the bars use.
